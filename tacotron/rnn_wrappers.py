...
def _compute_attention(attention_mechanism, cell_output, previous_alignments, attention_layer, is_manual_attention,
                       manual_alignments, time):
    computed_alignments, next_attention_state = attention_mechanism(cell_output,
                                                                    state=previous_alignments)  # (query, state)를 넘긴다.
    batch_size, max_time = tf.shape(computed_alignments)[0], tf.shape(computed_alignments)[1]

    alignments = tf.cond(is_manual_attention, lambda: manual_alignments[:, time, :],
                         lambda: computed_alignments, )  # 여기 이곳만 tensorflow 1.3과 다름.
...
class BahdanauMonotonicAttention_hccho(_BaseMonotonicAttentionMechanism):
    ...
    def __call__(self, query, state):
        ...
        with tf.variable_scope(None, "bahdanau_monotonic_hccho_attention", [query]):
            ...
            score_bias = tf.get_variable("attention_score_bias", dtype=processed_query.dtype,
            ...
            alignments_bias = tf.get_variable("alignments_bias", shape=(1), dtype=processed_query.dtype,
    ...
    def __call__(self, query, state):
        ...
        with tf.variable_scope(None, "Location_Sensitive_Attention", [query]):
            ...
            processed_query = tf.expand_dims(processed_query,
                                             1)  # _bahdanau_score처럼 _location_sensitive_score에서 해도 되는데, 여기서 했네.
        ...
        alignments = self._probability_fn(energy,
                                          previous_alignments)  # location sensitivity attention에서는 softmax가 아니고, smoothing normalization function을 사용한다.
    ...
    def __call__(self, query, state):
        with tf.variable_scope("GmmAttention"):
            ...
            mu = tf.reshape(tf.cast(tf.range(self.alignments_size), dtype=tf.float32),
                            shape=[1, 1, self.alignments_size])  # [[[0,1,2,...]]]
