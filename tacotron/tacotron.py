...
class Tacotron(nn.Module):
    ...
    def parse_batch(self, batch):
        ...
        return ((text_padded, input_lengths, mel_padded, max_len, speaker_id, output_lengths),
...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        for _ in range(hparams['enc_conv_num_layers']):
            conv_layer = nn.Sequential(
                ConvNorm(hparams['enc_conv_channels'],
                         hparams['enc_conv_channels'],
                         kernel_size=hparams['enc_conv_kernel_size'], stride=1,
                         padding=int((hparams['enc_conv_kernel_size']- 1) / 2),
                         dilation=1, w_init_gain='relu'),
                nn.BatchNorm1d(hparams['enc_conv_channels']))
            convolutions.append(conv_layer)
        self.convolutions = nn.ModuleList(convolutions)

        self.lstm = nn.LSTM(hparams['enc_conv_channels'],
                            int(hparams['enc_conv_channels'] / 2), 1,
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.encoder_embedding_dim = hparams['enc_conv_channels']
        ...
        self.attention_rnn = nn.LSTMCell(
            hparams['prenet_dim'] + hparams['enc_conv_channels'],
            hparams['attention_rnn_dim'])

        self.attention_layer = Attention(
            hparams['attention_rnn_dim'], hparams['enc_conv_channels'],
        ...
        self.linear_projection = LinearNorm(
            hparams['decoder_rnn_dim'] + hparams['enc_conv_channels'],
            hparams['n_mel_channels'] * hparams['n_frames_per_step'])

        self.gate_layer = LinearNorm(
            hparams['decoder_rnn_dim'] + hparams['enc_conv_channels'], 1,
